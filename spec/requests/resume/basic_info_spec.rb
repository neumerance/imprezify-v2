...
feature 'Resume Edit Form', :js do
  ...
  scenario 'filling up basic info fields' do
    ...
    within('#basic-info') do
      ...
      execute_script('document.getElementById("resume_basic_info_attributes_birthdate").value = "1987-04-15";')
    ...
    wait_for_ajax
    %w(photo name title profile_overview gender nationality religion address city country postal_code).each do |f|
      expect(resume.reload.basic_info.try(f.to_sym)).not_to be_nil
    end
  end
end
